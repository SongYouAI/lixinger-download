...
>
> **坑 3（🔴 未决）：公告列表是「最新在前 + 分页」的，会只抓到最近一年。**
> 实测去 search-key 后出现「发现 **1** 个年报链接, 覆盖年份: **2025**」——
> 2016~2024 在后续页，而脚本会当作"已最新"照常收工。
> **这是目前已知的最大缺口**，尚未修（需要处理分页：翻页 或 用页面上的"年度报告/定期报告"分类标签）。
> 已加的兜底：**覆盖度自检** —— 拿「库内已有年份」比对「本次发现年份」，
> 出现缺口就打 `🔴 覆盖度告警: 库内已有但本次未发现的年份: …`，避免静默漏抓。
> ⚠️ 首次下载（库为空）时该判据无效，仍需人工留意"发现年份数"是否明显偏少。
...
| `scripts/lib_pdf.sh` | **PDF 完整性校验库**（两脚本共用）：文件头 / 体积 / 尾部三判据 + 三个实测坑的规避 |
