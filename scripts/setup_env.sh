...
# ---- 6. 登录态（可自动恢复）----
echo "[6/6] 理杏仁登录态"
echo "      ℹ️  登录态过期无需手动处理：download_company.sh 检测到未登录时会"
echo "         自动点击右上角「登录/注册」(账号密码已存于浏览器) → 一点即登录。"
echo "      ⛔ 切勿因未登录就换数据源(东财/巨潮/上交所/新浪均不可取, 实测全部失败)。"
echo "      仅当自动登录失败时, 才手动在浏览器打开 https://www.lixinger.com 登录一次。"
